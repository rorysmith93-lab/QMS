import { setAuthorization, setWorkflowMode } from "@/app/dashboard/authorization/actions";
...
export default async function AuthorizationPage({
  ...
  const levelByCategoryAndProfile = new Map(
    (authRows ?? []).map((a) => [`${a.category}:${a.profile_id}`, a.level])
  );
  const memberList = members ?? [];

  return (
    <div>
      <h1 className="text-2xl font-bold text-[var(--text-primary)]">Authorization</h1>
      <p className="mt-1 text-sm text-muted">
        For each category: how it&apos;s approved, and who&apos;s authorized to author, check, or approve
        it. Covers Documents, Work Instructions, and SOPs — anything built through those modules is
        gated by the same matrix here. A category with nobody assigned stays open to everyone,
        nothing is restricted until you set it here.
      </p>
      ...
      <div className="mt-6 space-y-6">
        {DOCUMENT_CATEGORIES.map((cat) => {
          const boundSetMode = setWorkflowMode.bind(null, cat.value);
                ...
                <div className="overflow-x-auto"><table className="mt-4 w-full text-left text-sm">
                  ...
                  <tbody>
                    {memberList.map((member) => {
                      const boundSetAuth = setAuthorization.bind(null, cat.value, member.id);
